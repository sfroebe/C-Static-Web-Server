...
void process_client_socket(int epoll_fd, int client_fd);
...
int main(void)
{
    ...
    if (!add_epoll_fd(epoll_fd, server_fd)) {
        close(epoll_fd);
        close(server_fd);
        return EXIT_FAILURE;
    }
...
void accept_new_connections(int epoll_fd, int server_fd)
{
    while (1) {
        ...
        if (!set_nonblocking(client_fd) || !add_epoll_fd(epoll_fd, client_fd)) {
            close(client_fd);
            continue;
        }
...
void process_client_socket(int epoll_fd, int client_fd)
{
    /* This server has Connection: close semantics: one ready client means one
       request, one response, then one closed descriptor. Keeping that policy
       avoids the per-connection state machine required for HTTP keep-alive. */
    ...
    while (1) {
        ...
        for (i = 0; i < ready_count; i++) {
            ...
            if (fd == server_fd) {
                ...
                process_client_socket(epoll_fd, fd);
...
void handle_client(int client_fd)
{
    ...
    if (bytes_received == -1) {
        /* Read readiness is a hint, not a promise. With non-blocking sockets
           EAGAIN means there is no complete read to do right now; this simple
           one-request server closes rather than keeping connection state. */
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            perror("recv failed");
        }
        return;
    ...
    if (bytes_received == 0) {
        /* recv() returning zero means the peer closed before sending a request. */
        return;
    }
